...
HRESULT IQNet::AddLocalPlayerByUserIndex(DWORD dwUserIndex)
{
	...
	{
		IQNetPlayer& p = m_player[dwUserIndex];
		p.m_smallId     = (BYTE)dwUserIndex;
		p.m_isRemote    = false;
		p.m_isHostPlayer = (dwUserIndex == 0);
		swprintf_s(p.m_gamertag, 32, L"Player%d", (int)dwUserIndex);
		// Mark the slot active so Win64_IsActivePlayer returns true for index >= 1
		if (dwUserIndex >= s_playerCount)
			s_playerCount = dwUserIndex + 1;
		if (p.GetCustomDataValue() == 0)
			p.SetCustomDataValue(1);
	}
	return S_OK;
}
