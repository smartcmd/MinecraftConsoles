...
static int RunHeadlessServer()
{
	...
	// Set game options from server.properties before HostGame so advertising has correct values 
	int diffVal = serverSettings.getInt(L"difficulty", 1); 

	app.SetGameHostOption(eGameHostOption_Difficulty, diffVal);
	app.SetGameHostOption(eGameHostOption_Gamertags, 1);
	int gamemodeVal = serverSettings.getInt(L"gamemode", 0); 
	app.SetGameHostOption(eGameHostOption_GameType, gamemodeVal);
	...
	printf("Server settings: difficulty=%d gamemode=%d pvp=%d structures=%d fire=%d tnt=%d\n",
				diffVal, gamemodeVal,
				serverSettings.getBoolean(L"pvp", true) ? 1 : 0,
				serverSettings.getBoolean(L"generate-structures", true) ? 1 : 0,
				serverSettings.getBoolean(L"fire-spreads", true) ? 1 : 0,
				serverSettings.getBoolean(L"tnt", true) ? 1 : 0);
	printf("Packed settings bitmask: 0x%08X\n", app.GetGameHostOption(eGameHostOption_All));
	fflush(stdout); 
