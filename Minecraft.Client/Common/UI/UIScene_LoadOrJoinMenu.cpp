...
static wstring ReadCachedLevelName(const wstring& saveDir)
{
    ...
    HANDLE hFile = CreateFileW(cachePath.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0, nullptr);
    if (hFile == INVALID_HANDLE_VALUE) return L"";
    wchar_t buf[256] = {};
    DWORD bytesRead = 0;
    ReadFile(hFile, buf, sizeof(buf) - sizeof(wchar_t), &bytesRead, nullptr);
    CloseHandle(hFile);
...
{
    wstring cachePath = saveDir + L"\\levelName.cache";
    HANDLE hFile = CreateFileW(cachePath.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, 0, nullptr);
    if (hFile == INVALID_HANDLE_VALUE) return;
    DWORD written = 0;
    WriteFile(hFile, name.c_str(), static_cast<DWORD>(name.size() * sizeof(wchar_t)), &written, nullptr);
    CloseHandle(hFile);
}
...
static wstring ReadLevelNameFromSaveFile(const wstring& filePath)
{
    ...
    {
        wstring sidecarPath = saveDir + L"\\worldname.txt";
        FILE *fr = nullptr;
    ...
    // Check sidecar cache (instant for repeat loads)
    wstring cached = ReadCachedLevelName(saveDir);
    if (!cached.empty()) return cached;

    // Open the save file
    HANDLE hFile = CreateFileW(filePath.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_FLAG_RANDOM_ACCESS, nullptr);
    if (hFile == INVALID_HANDLE_VALUE) return L"";
    ...
    DWORD bytesRead = 0;
    if (!ReadFile(hFile, header, 8, &bytesRead, nullptr) || bytesRead < 8) { CloseHandle(hFile); return L""; }
    ...
    else
    {
            ...
            headerOffset + numEntries * entrySize <= fileSize)
        {
            ...
            SetFilePointer(hFile, headerOffset, nullptr, FILE_BEGIN);
            unsigned int tableSize = numEntries * entrySize;
            unsigned char *tableData = new unsigned char[tableSize];
            if (ReadFile(hFile, tableData, tableSize, &bytesRead, nullptr) && bytesRead == tableSize)
            {
                ...
                for (unsigned int i = 0; i < numEntries; i++)
                {
                    if (wcscmp(table[i].filename, L"level.dat") == 0)
                    {
                        ...
                        if (off >= 12 && off + len <= fileSize && len > 0 && len < 4 * 1024 * 1024)
                        {
                            ...
                            DWORD levelRead = 0;
                            if (ReadFile(hFile, levelData, len, &levelRead, nullptr) && levelRead == len)
                            {
                                byteArray ba;
